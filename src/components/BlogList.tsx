...
export default function BlogList({ posts }: { posts: ViewPost[] }) {
          ...
          >
            ...
            <div className="p-5">
              <h2 className="font-display text-lg font-semibold leading-snug transition group-hover:text-ember">
                {p.title[contentLocale]}
              </h2>
              <p className="mt-2 line-clamp-2 text-sm text-sand/60">{p.excerpt[contentLocale]}</p>
            </div>
          </Link>
        ))}
      </div>
