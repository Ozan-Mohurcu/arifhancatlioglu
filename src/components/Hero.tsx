"use client";

import { motion } from "framer-motion";
import dynamic from "next/dynamic";
import { profile, currentLocation, stats, upcoming } from "@/data/site";
import { Heart, ChevronDown } from "./Icons";

// Harita yalnızca tarayıcıda yüklensin
const WorldMap = dynamic(() => import("./WorldMap"), { ssr: false });

export default function Hero() {
  return (
    <section id="yolculuk" className="relative min-h-[100svh] w-full overflow-hidden">
      {/* Arka plan: 3D globe */}
      <div className="absolute inset-0">
        <WorldMap />
      </div>

      {/* Okunabilirlik için degrade */}
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-ink/70 via-ink/10 to-ink" />

      {/* İçerik */}
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-6xl flex-col justify-end px-5 pb-16 pt-28 md:justify-center">
        <motion.div
          initial={{ opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, ease: "easeOut" }}
          className="max-w-xl"
        >
          {/* Canlı konum rozeti */}
          <div className="eyebrow mb-4">
            <span className="relative flex h-2.5 w-2.5">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-ember opacity-75" />
              <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-ember" />
            </span>
            Şu an: {currentLocation.city}, {currentLocation.country}
          </div>

          <h1 className="font-display text-4xl font-extrabold leading-tight sm:text-5xl md:text-6xl">
            {profile.name.split(" ")[0]} <span className="text-gradient">yollarda</span>
          </h1>
          <p className="mt-4 text-base text-sand/80 sm:text-lg">{profile.tagline}</p>

          {/* İstatistikler */}
          <div className="mt-8 flex flex-wrap gap-6">
            {stats.map((s) => (
              <div key={s.label}>
                <div className="font-display text-2xl font-bold text-ember sm:text-3xl">
                  {s.value}
                </div>
                <div className="text-xs uppercase tracking-wider text-sand/60">{s.label}</div>
              </div>
            ))}
          </div>

          {/* CTA */}
          <div className="mt-8 flex flex-wrap gap-3">
            <a
              href="#destek"
              className="inline-flex items-center gap-2 rounded-full bg-ember px-6 py-3 font-semibold text-ink shadow-lg shadow-ember/20 transition hover:bg-amber"
            >
              <Heart size={18} /> Yolculuğa Destek Ol
            </a>
            <a
              href="#hakkinda"
              className="glass rounded-full px-6 py-3 font-semibold transition hover:bg-white/10"
            >
              Hikâyemi Oku
            </a>
          </div>
        </motion.div>
      </div>

      {/* Aşağı kaydır göstergesi */}
      <div className="absolute bottom-5 left-1/2 z-10 -translate-x-1/2 animate-floaty text-sand/40">
        <ChevronDown size={22} />
      </div>

      {/* Harita lejantı */}
      <div className="glass absolute right-5 top-24 z-10 hidden rounded-xl p-3.5 text-xs md:block">
        <div className="mb-1.5 flex items-center gap-2.5">
          <span className="inline-block h-2 w-2 rounded-full bg-moss" /> Gittiğim yerler
        </div>
        {upcoming.length > 0 && (
          <div className="mb-1.5 flex items-center gap-2.5">
            <span className="inline-block h-2 w-4 rounded-full bg-sky" /> Gelecek rotam
          </div>
        )}
        <div className="flex items-center gap-2.5">
          <span className="inline-block h-2.5 w-2.5 rounded-full bg-ember" /> Şimdiki konumum
        </div>
      </div>
    </section>
  );
}
